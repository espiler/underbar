...
(function() {
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(item) {
      if (typeof(functionOrKey) == 'function') return functionOrKey.apply(item, arguments);
      else return item[functionOrKey].apply(item, arguments)
    });
  };
  ...
  _.every = function(collection, iterator) {
    ...
    var result = _.reduce(collection, function(current, item) {
  ...
  _.defaults = function(obj) {
    for (var i=0; i<arguments.length; i++) {
      for (var key in arguments[i]) {
        if (obj[key] == null) obj[key] = arguments[i][key];
      }
    }
    return obj;
  };
  ...
  _.shuffle = function(array) {
    ...
    while (arrayCopy.length > 0) {
      var rand = Math.floor(Math.random()*arrayCopy.length);
      result.push(arrayCopy[rand]);
